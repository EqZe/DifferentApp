...
export default function ProfileScreen() {
  ...
  const handleTestNotification = async () => {
    console.log('User tapped Test Notification button - sending all 3 reminder types');
    ...
    try {
      ...
      console.log('Fetching user tasks for test notifications...');
      ...
      if (tasks && tasks.length > 0) {
        ...
        console.log('Selected random task for test:', taskTitle);
      } else {
        console.log('No tasks found, using default task title');
      }

      // Send all 3 types of reminders
      console.log('Sending all 3 test reminders for task:', taskTitle);
      await sendTestTaskReminders(taskTitle);
      
      console.log('✅ All 3 test notifications sent successfully');
    } catch (error: any) {
      console.error('❌ Error sending test notifications:', error?.message || error);
    } finally {
      setIsSendingNotification(false);
    }
  };
