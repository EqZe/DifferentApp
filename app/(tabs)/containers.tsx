...
export default function ContainersScreen() {
  ...
  const loadContainers = useCallback(async () => {
    if (!user?.id) {
      console.log('ContainersScreen: No user ID, skipping load');
      setLoading(false);
      return;
    }

    try {
      console.log('ContainersScreen: Loading containers for user', user.id);
      const data = await api.getContainers(user.id);
      console.log('ContainersScreen: Loaded containers:', data.length);
      setContainers(data);
    } catch (error: any) {
      console.error('ContainersScreen: Failed to load containers', error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const onRefresh = useCallback(() => {
    console.log('ContainersScreen: Refreshing containers');
    setRefreshing(true);
    loadContainers();
  }, [loadContainers]);
