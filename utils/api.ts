...
export const api = {
  ...
  signUp: async (email: string, password: string, fullName: string, city: string, phoneNumber?: string): Promise<UserFrontend> => {
    console.log('signUp pressed', { email, fullName });
  ...
  updateUser: async (
    ...
    console.log('updateUser request', { authUserId, updates });
  ...
  getCategories: async (): Promise<CategoryWithPosts[]> => {
    ...
    const categoriesWithPosts: CategoryWithPosts[] = categoriesData?.map((category) => {
      ...
      return {
        ...
        coverImage: stats.coverImage || camelCategory.coverImage,
  ...
  createTask: async (
    ...
    console.log('createTask request', { authUserId, task });
    const { data, error } = await supabase
      .from('user_tasks')
      .insert({
        auth_user_id: authUserId,
        task_metadata_id: null,
  ...
  completeTask: async (taskId: string, requiresPending: boolean, currentStatus: 'YET' | 'PENDING' | 'DONE'): Promise<TaskFrontend> => {
    console.log('completeTask request', { taskId, requiresPending, currentStatus });
    const newStatus = requiresPending
    ...
    if (error) throw new Error(error.message || 'שגיאה בעדכון משימה');

    const metadata = (data as any).tasks_metadata;
    return {
      id: data.id,
      userId: data.auth_user_id,
      title: metadata?.title || 'משימה ללא כותרת',
  ...
  deleteTask: async (taskId: string): Promise<void> => {
    console.log('deleteTask request', { taskId });
    const { error } = await supabase
      .from('user_tasks')
      .delete()
      .eq('id', taskId);
  ...
  createContainer: async (
    ...
    console.log('createContainer request', { authUserId, containerIdPerUser });
    const { data, error } = await supabase
      .from('user_containers')
      .insert({
        auth_user_id: authUserId,
  ...
  updateContainer: async (
    ...
    console.log('updateContainer request', { containerId, updates });
    const updateData: any = {};
  ...
  deleteContainer: async (containerId: string): Promise<void> => {
    console.log('deleteContainer request', { containerId });
    const { error } = await supabase
      .from('user_containers')
      .delete()
      .eq('id', containerId);
...
export async function registerOneSignalPlayer(
  ...
  console.log('registerOneSignalPlayer request', { playerId, authUserId });
